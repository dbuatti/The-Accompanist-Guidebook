...
export default function CurriculumToolsMenu({ onRefetch }: CurriculumToolsMenuProps) {
  ...
  return (
    <>
        ...
        <DropdownMenuContent align="end" className="w-64">
          {tools.slice(0, -1).map((tool) => (
            <DropdownMenuItem
              ...
              disabled={!!pending}
              className={tool.destructive ? "text-destructive focus:text-destructive" : undefined}
            >
              {tool.label}
            </DropdownMenuItem>
          ))}
      ...
      <ConfirmDialog
        open={!!confirmTool}
        onOpenChange={(open) => { if (!open) setConfirmTool(null); }}
        title={confirmTool?.label || ""}
        description={confirmTool?.confirmMessage}
        confirmLabel={confirmTool?.label}
        destructive={confirmTool?.destructive}
        isPending={confirmTool ? pending === confirmTool.key : false}
        onConfirm={() => {
          if (confirmTool) runTool(confirmTool);
          setConfirmTool(null);
        }}
      />
    </>
